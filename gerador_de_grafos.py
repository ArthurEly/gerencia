...
# --- PARTE 1: TRADUTOR (COM TODAS AS CORREÇÕES) ---
def mib_para_rdf(mib_builder, mib_name):
    ...
    # --- CORREÇÃO 1: Separar as chamadas de import_symbols ---
    # Importa de SNMPv2-SMI
    MibScalar, MibTableColumn, MibTableRow = mib_builder.import_symbols(
        'SNMPv2-SMI', 'MibScalar', 'MibTableColumn', 'MibTableRow'
    )
    # Importa de SNMPv2-CONF
    ...
    for symbol_name, symbol_obj in symbols.items():
        ...
        # --- LÓGICA DE CLASSIFICAÇÃO (Correta) ---
        # Checa por MibTableColumn PRIMEIRO, pois ele é um subtipo de MibScalar
        
        if isinstance(symbol_obj, MibTableColumn):
            # É UMA COLUNA DE TABELA
            g.add((subject, RDF.type, MIBO.MibObject))
            g.add((subject, RDF.type, MIBO.TableColumn))
            column_count += 1
            
        elif isinstance(symbol_obj, MibScalar):
            # É UM ESCALAR (E NÃO UMA COLUNA)
            g.add((subject, RDF.type, MIBO.MibObject))
            g.add((subject, RDF.type, MIBO.Scalar))
            scalar_count += 1
            ...
            # Não é um objeto que queremos (ex: MibTableRow, MibTable)
            continue

        # Adiciona os atributos (só para Escalares e Colunas)
    ...
    print(f"  -> Classificados {scalar_count} Escalares e {column_count} Colunas de Tabela.")
    return g

# --- PARTE 2: VISUALIZADOR (BUG CORRIGIDO) ---
def visualizar_interativo(rdf_graph, descricoes, output_filename):
    ...
    # --- AQUI ESTAVA O BUG (LINHA 82) ---
    # Corrigido de isinstance(s, URIRef) para isinstance(o, URIRef) na segunda metade
    ...
    for uri in all_uris:
        ...
        all_types = [t.split('#')[-1] for t in rdf_graph.objects(subject=uri, predicate=RDF.type) if isinstance(t, URIRef)]
        type_text = ", ".join(sorted(all_types)) if all_types else "Tipo não definido"
        ...
        for p, o in rdf_graph.predicate_objects(subject=uri):
            if p not in [RDF.type, RDFS.comment, Namespace("http://purl.org/net/mibo#")["isMemberOf"]]:
                ...
                obj_value = str(o)
                if prop_name == "hasValue":
                    prop_name = "**VALOR (Vivo)**"
...
if __name__ == "__main__":
    ...
    # --- CORREÇÃO: Usando snake_case para pysnmp v6+ ---
    ...
    for mib_name in mibs_para_processar:
        ...
        try:
            ...
            print(f"-> Grafo RDF (schema) salvo em '{rdf_filename}'.")
    ...
    print("\n--- Processo (Etapa 1: Geração de Schema) concluído! ---")